...
class DronesController extends Controller
{
    ...
    {
        //
        $drones = Drones::latest()->paginate(10);
        $drones->each(function ($drone) {
            if ($drone->type !== 'link') {
                $drone->path = Storage::url($drone->path);
            }
        });
        return Inertia::render('dashboard/Drones', ['drones' => $drones]);
    }
        ...
        try {
            ...
            try {
                ...
                if ($request->type === 'upload') {
                    // Store the file
                    $uploadedFilePath = $request->file('file')->store('uploads/drones', 'public');
                ...
                // Create the record
                $story =  Drones::create($validated);

                // If we got here, everything worked - commit the transaction
                DB::commit();

                return redirect()->route('drones.index')->with('success', 'Image added successfully.');
            } catch (\Exception $e) {
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            try {
                ...
                return to_route('drones.index')->with('success', 'Image updated successfully.');
            } catch (\Exception $e) {
    ...
    public function destroy(string $id)
    {
        ...
        if ($drone->type !== 'link') {
            Storage::disk('public')->delete($drone->path);
        }
