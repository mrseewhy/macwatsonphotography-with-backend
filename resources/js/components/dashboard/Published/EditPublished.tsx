...
const EditPublished = ({ imageToEdit, setShowEdit }) => {
    const { data, setData, errors, processing, post, reset } = useForm({
        _method: 'PUT',
        title: imageToEdit.title || '',
        description: imageToEdit.description || '',
        type: imageToEdit.type || 'link',
    ...
    useEffect(() => {
        ...
        if (imageToEdit.type === 'link' && imageToEdit.link) {
            setPreview(imageToEdit.link);
    ...
    useEffect(() => {
        if (data.type === 'link') {
            ...
            } else if (imageToEdit.type === 'link' && imageToEdit.link) {
            ...
            if (data.file) {
                ...
                setIsValidImage(true);
            } else if (imageToEdit.type === 'upload' && imageToEdit.path && !preview) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('published.update', data.id), {
            preserveScroll: true,
            preserveState: true,
            forceFormData: true,
            onSuccess: () => {
    ...
    return (
        <div className="mx-auto w-full max-w-xl rounded-lg bg-white p-8">
            <h2 className="mb-8 text-2xl font-bold text-gray-800">Edit Image in Published Works</h2>
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            if (newType === 'link' && imageToEdit.link) {
                                setPreview(imageToEdit.link);
                            } else if (newType === 'upload' && imageToEdit.path) {
                                setPreview(imageToEdit.path);
                            }
                        }}
                        className={`w-full rounded-md border px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none ${
