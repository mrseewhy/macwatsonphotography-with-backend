...
const PublishedTable: React.FC<{ stories: StoryPagination }> = ({ allPublished }) => {
    ...
    const selectedStory = allPublished.data.find((story) => story.id === selectedStoryId) ?? null;

    const handleEdit = (id: number) => {
        const mystory = allPublished.data.find((story) => story.id === id);
    ...
    const confirmDelete = () => {
        destroy(route('published.destroy', idToDelete), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <div className="w-full overflow-x-auto">
            ...
            ) : allPublished.data.length === 0 ? (
                <div className="py-10 text-center text-gray-500">No Published Work found.</div>
            ) : (
                <div className="inline-block min-w-full overflow-hidden rounded-lg border border-gray-300">
                    <table className="w-full min-w-[600px]">
                        ...
                        <tbody className="text-sm">
                            {allPublished.data.map((story, index) => (
            ...
            {/* Pagination */}
            {!loading && allPublished && (
                <div className="mt-6 flex items-center justify-center gap-2">
                    {allPublished.links.map((link, index) => (
                        <button
                            key={index}
                            disabled={!link.url}
            ...
            {showEdit !== null && allPublished.data.find((story) => story.id === showEdit) && (
                <Modal show={true} onClose={() => setShowEdit(null)}>
                    ...
                    /> */}
                    <EditPublished
                        story={allPublished.data.find((story) => story.id === showEdit)!}
                        onClose={() => setShowEdit(null)}
                        imageToEdit={imageToEdit}
                        setShowEdit={setShowEdit}
                    />
            ...
            {showDelete !== null && (
                <Modal show={true} onClose={() => setShowDelete(null)}>
                    <div className="mx-auto w-full max-w-xl rounded-lg bg-white p-8">
                        ...
                        <p>Are you sure you want to delete this published work image?</p>
